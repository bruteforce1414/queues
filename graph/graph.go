...
func (g *DirectedGraph) AddNodeThroughSetEdge(n Node) {
	//проверим, есть ли нода с таким же идентификатором в графе
	if g.Has(n) {
		fmt.Println("Вершина с идентификатором '", n.Id(), "' уже существует. Вершина не была добавлена к графу")
		return
...
func (g *DirectedGraph) RemoveNode(n Node) {
	...
	for _, value := range g.nodes {
		...
		if g.HasEdgeFromTo(value, n) {
			delete(g.from[value.Id()], n.Id())
			if len(g.from[value.Id()]) == 0 {
				delete(g.from, n.Id())
			}
		}
	}
	...
	for _, value := range g.nodes {
		...
		if _, ok := g.to[value.Id()][n.Id()]; ok {
			{
				delete(g.to[value.Id()], n.Id())
				if len(g.to[value.Id()]) == 0 {
					delete(g.to, n.Id())
				}
			}
		}
...
func (g *DirectedGraph) SetEdge(e Edge) {
	...
	if !g.Has(e.From()) {
		g.AddNodeThroughSetEdge(e.From())
	}
	if !g.Has(e.To()) {
		g.AddNodeThroughSetEdge(e.To())
	}
